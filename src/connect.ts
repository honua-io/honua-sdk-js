...
import { snapshotCacheData, validateCachedEvidence } from "./connect-cache-data.js";
import {
...
import {
  ...
  awaitAbortable,
  isPmtilesSchemeInput,
  validateConnectEndpoint,
} from "./connect-endpoint.js";
...
import { normalizePmtilesDiscoveryLimits } from "./connect-pmtiles.js";
...
import { canonicalizeUrlQuery, isCredentialQueryName } from "./connect-url-safety.js";
...
export async function connectWithSourceSchemaProjection(
  ...
  let client: HonuaClient;
  let snapshot: ConnectDiscoverySnapshot | undefined;
  let cacheStatus: ConnectCacheStatus;

  if (target.protocol === "pmtiles") {
    const focused = await inspectPmtilesDiscovery(options, {
      limits: pmtilesLimits,
      ...(focusedProjectionIdentity ? { projectionIdentity: focusedProjectionIdentity } : {}),
      ...(sourceSchemaProjection ? { sourceSchemaProjection } : {}),
    });
    identity = focused.identity;
    client = focused.client;
    snapshot = focused.snapshot;
    cacheStatus = focused.cacheStatus;
  } else {
    identity = await createDiscoveryCacheIdentity({
    ...
    client = options.client ?? new HonuaClient({ ...options.clientOptions, baseUrl: target.clientBaseUrl });
    if (!snapshot) {
      snapshot =
...
function validateCachedDiscoveryMetadata(value: DiscoverySourceMetadata, endpoint: string): DiscoverySourceMetadata {
  ...
  const tileMatrixSets =
    value.tileMatrixSets !== undefined
      ...
      : undefined;
  if (value.pmtiles !== undefined)
    cacheMetadataError("Cached PMTiles metadata is present outside a PMTiles discovery boundary.");
