...
export function compileDuckDbLosslessRowDecoder(fields: readonly SourceProfileField[]): DuckDbLosslessRowDecoder {
  ...
  try {
    ...
    rowFailure("GEOPARQUET_LOSSLESS_AMBIGUOUS_WRAPPER", "$.fields", "ROW");
  }
  if (!isArray) {
    throw new DuckDbLosslessDecodeError("GEOPARQUET_LOSSLESS_CONTAINER_LIMIT", "$", "ROW");
  }
  const fieldReader = arrayReader(
    fields as SourceProfileField[],
    { declaredType: "ROW", ancestors: new WeakSet(), entries: 0 },
    "$.fields",
  );
  const names = new Set<string>();
  const compiled = Array.from({ length: fieldReader.length }, (_, index) => {
    const field = normalizeSourceProfileField(fieldReader.get(index), index);
    if (field.name.length === 0 || field.name.length > MAX_TYPE_LENGTH || names.has(field.name)) {
...
function normalizeSourceProfileField(value: unknown, index: number): Readonly<SourceProfileField> {
  const path = `$.fields[${index}]`;
  const state: DecodeState = { declaredType: "ROW", ancestors: new WeakSet(), entries: 0 };
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    rowFailure("GEOPARQUET_LOSSLESS_INVALID_TYPE", path, "ROW");
  }
  if (!isPlainRecord(value, state, path)) {
    rowFailure("GEOPARQUET_LOSSLESS_AMBIGUOUS_WRAPPER", path, "ROW");
  }
  const entries = new Map(plainRecordEntries(value, state, path));
  const name = entries.get("name");
  const type = entries.get("type");
  const nullable = entries.get("nullable");
  if (
    typeof name !== "string" ||
    typeof type !== "string" ||
    (entries.has("nullable") && nullable !== undefined && typeof nullable !== "boolean")
  ) {
    rowFailure("GEOPARQUET_LOSSLESS_INVALID_TYPE", path, "ROW");
  }
  return Object.freeze({ name, type, ...(typeof nullable === "boolean" ? { nullable } : {}) });
}
...
function encodeBinary(value: unknown, state: DecodeState, path: string): string {
  const bytes = copyByteTypedArrayBytes(value) ?? copyArrayBufferBytes(value);
...
function copyTypedArrayBytes(value: unknown): Uint8Array | undefined {
  ...
  try {
    const tag = Reflect.apply(typedArrayTag, value, []) as string | undefined;
    if (!tag || tag === "Float32Array" || tag === "Float64Array") return undefined;
...
function copyByteTypedArrayBytes(value: unknown): Uint8Array | undefined {
  if (typeof value !== "object" || value === null || !ArrayBuffer.isView(value)) return undefined;
  try {
    const tag = Reflect.apply(typedArrayTag, value, []) as string | undefined;
    if (tag !== "Uint8Array" && tag !== "Uint8ClampedArray") return undefined;
