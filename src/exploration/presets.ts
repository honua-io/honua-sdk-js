...
export const LINKED_VIEW_PRESETS: Readonly<Record<LinkedViewPresetName, LinkedViewPolicy>> = {
  ...
  chartDriven: {
    ...
    rules: roleRules({
      map: NO_SLICES,
      grid: NO_SLICES,
      chart: ["grouping", "aggregation", "filters", "selection"],
      filter: ["filters"],
      detail: NO_SLICES,
      form: NO_SLICES,
      custom: NO_SLICES,
...
function roleRules(table: Record<ViewRole, ReadonlyArray<ExplorationSlice>>): LinkedViewPolicy["rules"] {
  return [
    ...
    { role: "filter", propagatesSlices: table.filter },
    { role: "detail", propagatesSlices: table.detail },
