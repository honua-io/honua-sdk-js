/**
 * Minimal, dependency-free Server-Sent-Events frame parser.
 *
 * **Origin.** Ported verbatim from `honua-studio`'s `src/chat/sse-parser.ts`
 * (honua-studio#6), where it is zero-import, DOM-free, and node-tested.
 *
 * It exists as its own module — independent of `fetch`/`ReadableStream` — so
