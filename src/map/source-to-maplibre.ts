...
import { toJsonValue } from "../query-planner/canonical.js";
import { queryFromCanonical, queryIrSourceIdentity } from "../query-planner/ir.js";
import { hashQueryPlanV1, hashQueryPlanV1WithSubtleCrypto } from "../query-planner/plan-integrity.js";
import {
...
function assertProjectionPlanContext<T>(
  ...
  const runtimeKey = JSON.stringify(toJsonValue(runtimeCapabilities));
  if (
    JSON.stringify(toJsonValue(identity)) !== JSON.stringify(toJsonValue(plan.ir.source)) ||
    runtimeKey !== JSON.stringify(toJsonValue(descriptorCapabilities)) ||
    runtimeKey !== JSON.stringify(toJsonValue(planCapabilities))
  ) {
    throw new HonuaQueryPlanExecutionError(
...
function assertExecutionContext<T>(
  ...
  if (
    JSON.stringify(toJsonValue(queryIrSourceIdentity(source.descriptor, options))) !==
    JSON.stringify(toJsonValue(plan.ir.source))
  ) {
    throw new HonuaQueryPlanExecutionError(
      "plan-context-mismatch",
