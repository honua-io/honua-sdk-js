...
| Static STAC asset discovery | `experimental` | `standalone` | `discovery` | Any static STAC Catalog, Collection, or Item | discoverStaticStac() from @honua/sdk-js/stac-discovery | [fixture: stac-static-discovery-fixtures](../test/stac-discovery.test.ts) | Traverses bounded static links and emits fail-closed COG, GeoParquet, PMTiles, tile, and metadata candidates for explicit inspection before adapter execution. |
