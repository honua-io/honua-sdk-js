...
| `locator.raster` | additive locator metadata | Capability-reviewed WMS GetMap or WMTS KVP/ResourceURL binding selected by raw `connect()` discovery. The existing MapLibre raster projector executes this exact safe URL/template instead of guessing from `locator.url`; unknown locator fields are preserved by the SDK package runtime. Server-side executors may ignore it until their locator model adopts the same binding contract. |
...
- `locator.raster` carries only a same-origin, credential-free operation URL
  or template plus selected format/matrix pattern. It never carries request
  headers or tokens; discovery rejects credential-bearing query state before
  constructing it.
