...
[`docs/bundle-sizes.md`](./bundle-sizes.md) (measured 2026-07-17 at commit `a18e3f01`;
...
| --- | ---: | ---: |
| Full root entrypoint: connect → query → explain → mount workflow | 579.2 KiB | 153.6 KiB |
| Importing only `HonuaClient` (tree-shake guard) | 215.6 KiB | 55.1 KiB |
| Data→map bridge only: `mountSourceToMapLibre` from `/map` | 40.7 KiB | 12.8 KiB |
| Protocol-neutral contract (`Dataset`/`Source`/`Query`/`Result`) | 264.9 KiB | 70.1 KiB |
| ArcGIS compatibility layer (drop-in migration surface) | 983.2 KiB | 245.2 KiB |
...
our **uncompressed minified** bytes (engine 1024.9 KiB + Honua root 579.2 KiB ≈
1.57 MB) against Esri's reported startup JavaScript total
(3.5–4.1 MB) — the open stack ships roughly a third of the JavaScript, and
0.41 MB over the wire with gzip. Esri's totals also grow with widgets;
