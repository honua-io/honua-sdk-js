...
[`docs/bundle-sizes.md`](./bundle-sizes.md) (measured 2026-07-17 at commit `1bb4c044`;
...
| --- | ---: | ---: |
| Full root entrypoint: connect → query → explain → mount workflow | 469.4 KiB | 123.7 KiB |
| Importing only `HonuaClient` (tree-shake guard) | 210.5 KiB | 53.2 KiB |
| Data→map bridge only: `mountSourceToMapLibre` from `/map` | 36.0 KiB | 11.4 KiB |
...
our **uncompressed minified** bytes (engine 1024.9 KiB + Honua root 469.4 KiB ≈
1.46 MB) against Esri's reported startup JavaScript total
