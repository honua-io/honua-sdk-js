...
[`docs/bundle-sizes.md`](./bundle-sizes.md) (measured 2026-07-24 at commit `d6d46c34`;
...
| --- | ---: | ---: |
| Full root entrypoint: connect → query → explain → mount workflow | 615.5 KiB | 163.3 KiB |
| Importing only `HonuaClient` (tree-shake guard) | 217.9 KiB | 55.8 KiB |
| Data→map bridge only: `mountSourceToMapLibre` from `/map` | 42.4 KiB | 13.0 KiB |
| Protocol-neutral contract (`Dataset`/`Source`/`Query`/`Result`) | 289.4 KiB | 76.8 KiB |
| ArcGIS compatibility layer (drop-in migration surface) | 985.4 KiB | 245.7 KiB |
| Geocoding client | 26.7 KiB | 7.6 KiB |
| Routing client | 20.3 KiB | 6.3 KiB |
...
our **uncompressed minified** bytes (engine 1024.9 KiB + Honua root 615.5 KiB ≈
1.60 MB) against Esri's reported startup JavaScript total
