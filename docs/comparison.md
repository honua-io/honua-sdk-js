...
[`docs/bundle-sizes.md`](./bundle-sizes.md) (measured 2026-07-17 at commit `7345d932`;
...
| --- | ---: | ---: |
| Full root entrypoint: connect → query → explain → mount workflow | 554.7 KiB | 147.0 KiB |
| Importing only `HonuaClient` (tree-shake guard) | 212.0 KiB | 53.8 KiB |
| Data→map bridge only: `mountSourceToMapLibre` from `/map` | 40.7 KiB | 12.8 KiB |
| Protocol-neutral contract (`Dataset`/`Source`/`Query`/`Result`) | 286.4 KiB | 76.0 KiB |
| ArcGIS compatibility layer (drop-in migration surface) | 979.4 KiB | 243.6 KiB |
...
our **uncompressed minified** bytes (engine 1024.9 KiB + Honua root 554.7 KiB ≈
1.54 MB) against Esri's reported startup JavaScript total
