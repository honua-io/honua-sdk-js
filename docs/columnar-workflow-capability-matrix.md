...
| Decode Honua `geoarrow.wkb` Arrow IPC into bounded batches | Experimental GeoArrow 0.2 WKB subset with optional Apache Arrow peer; Binary/LargeBinary Point, LineString, or Polygon in XY/XYZ plus one object-id, one UTF-8/dictionary field, and one timestamp field. Embedded EWKB SRIDs are ignored; validated optional `crs`/`crs_type` field-extension metadata is preserved, while geometry kind comes from WKB values or schema-level GeoParquet `geometry_types` for empty responses. | Current FeatureServer `f=arrow` contract | Exact server-produced fixture only | `test/fixtures/columnar/honua-server-geoarrow-02-point.manifest.json` and `test/columnar-workflow-honua-arrow.test.ts` |
...
No row, byte, latency, or memory result in this matrix is a live-server claim. The WKB interop artifact and its exact producer receipt are bound to reviewed Honua Server head `66a9d34496c6f6a03dd571957062f773bfef7f0a`, merged as `4ef53ce7f49b78aad3572db1dfc3be88a6654a43`, and prove serialization compatibility only. A live status requires a pinned demo-manifest target and the same semantic verification used by the samples gallery.
