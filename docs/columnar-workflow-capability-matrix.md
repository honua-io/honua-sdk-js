...
| Decode Honua `geoarrow.wkb` Arrow IPC into bounded batches | Experimental GeoArrow WKB subset with optional Apache Arrow peer; Binary/LargeBinary Point, LineString, or Polygon in XY/XYZ plus one object-id, one UTF-8/dictionary field, and one timestamp field | Current FeatureServer `f=arrow` contract | Exact server-produced fixture only | `test/fixtures/columnar/honua-server-geoarrow-wkb.manifest.json` and `test/columnar-workflow-honua-arrow.test.ts` |
