...
| --- | --- | --- | --- | --- |
| `quickstart-feature-readonly` | `examples/maplibre-quickstart` | Anonymous FeatureServer `natural-earth`, layer `0`, supplied as one public endpoint through `VITE_HONUA_FIRST_MAP_URL`; protocol, source, filter, bound, and basemap use the reviewed `VITE_HONUA_FIRST_MAP_*` surface. No browser credential setting exists. | Same-origin fixture via `npm run demo:quickstart:mock`. | Fixture: `npm run test:playwright:quickstart`; cloud scaffold: `npm run test:cloud-demo:staging`; scheduled dual-protocol evidence: `npm run bench:live`. |
