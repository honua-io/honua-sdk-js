...
378 cells across 21 components x 18 gates: **312 passing**, 0 failing, 20 pending, 46 not applicable.

1 of 21 components have cleared every gate. That is a different axis from the catalog's `supportTier`, which records the functional maturity a feature issue delivered: 1 component(s) are `production-tier` and still carry open gates, listed per component as `openGates` in the manifest.
