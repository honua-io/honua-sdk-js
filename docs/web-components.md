...
- `honua-map-status`
- `honua-action-panel`
- `honua-time-slider` (binds to a `createTemporalPlayback` controller)
...
keeps its snapshot-plus-delta semantics unchanged, because the element adds
nothing between the controller and its filter sinks.

It binds through a property rather than `for="<map-id>"`, because the thing it
drives is the controller, not the map:

```html
<honua-map id="map"></honua-map>
<honua-time-slider id="time" label="Incident time"></honua-time-slider>
```
...
const playback = createTemporalPlayback({
  handle: mountedSource, // or `layer: { map, layerIds }`
  timeField: "event_time",
  extent: ["2026-06-01T00:00:00Z", "2026-07-01T00:00:00Z"],
...
Attributes: `label`, `speeds` (comma-separated rate multipliers, default
`0.5,1,2,4`), and `unavailable-reason`. Set `unavailable-reason` when the
mounted source has no usable temporal metadata: the panel still renders, names
the reason, and disables every transport control rather than offering a
scrubber that cannot move anything. The speed control is hidden entirely when
the bound controller exposes no `setSpeed`.

The scrubber follows the WAI-ARIA slider pattern — `role="slider"` with
`aria-valuemin` / `aria-valuemax` / `aria-valuenow` in epoch milliseconds, a
readable `aria-valuetext` naming the window, and a single tab stop. Keyboard:
Arrow keys step one window (the inline axis mirrors under `dir="rtl"`), Page
Up / Page Down move ten steps, Home / End jump to the first / last window.

`TimeSliderCompat` from `@honua/sdk-esri-compat` mounts this element into its
ArcGIS-style `container` once the application has called
`registerHonuaWidgetKit()`; without a registered kit the shim stays
state-model-only, exactly as before.
