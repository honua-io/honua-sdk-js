...
| `imagery-cog-quickstart` | `standalone` | Was `requires-live-backend`. With `VITE_HONUA_IMAGERY_BASE_URL` unset, `resolveImageryCogConfig` resolves mode `fixture-safe` against the document origin, and `normalizeBaseUrl` rejects any cross-origin, query-bearing, or credential-bearing override. |
...
`imagery-cog-quickstart` emits an exact synthetic CC0-1.0 tiled GeoTIFF as digest-pinned 64 KiB chunks. The sample's bundle-local adapter reconstructs only requested ranges as HTTP-compatible `206` responses, rejects missing or oversized ranges, and never publishes a complete-object route. No host fixture service is required.
