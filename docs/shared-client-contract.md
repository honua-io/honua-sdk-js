...
| `SourceLocator` | Protocol-specific endpoint info (`url`, `serviceId`, `layerId`, `collectionId`, `tileMatrixSetId`, `styleId`, `typeName`, `entitySet`, `taskName`, and the optional reviewed WMS/WMTS `raster` binding). It is a superset of the server `SourceBinding.locator`; additive route/binding hints are tracked in [`source-binding-alignment.md`](./source-binding-alignment.md). |
...
- `wmtsSource` — WMTS 1.0.0 (render + tiles via a reviewed ResourceURL/KVP
  binding for raw discovery or canonical RESTful tiles for Honua descriptors;
  query family throws; service / layer / tileset handles reachable via
