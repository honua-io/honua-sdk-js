...
- Data and auth: `hybrid` · `anonymous`
- Provenance: Same-origin OGC fixture observations or scheduled public GeoServices and OGC inspection receipts.
- Freshness: Fixture replay is deterministic; scheduled live evidence records its observation time.
- Catalog degradation: Discovery may be partial or an inspected source may be query/render unsupported; every unavailable action retains a structured evidence or planner reason.
...
- honua.io journey: `connect-existing-gis`
- Degradation: Discovery can succeed while query or render remains unavailable; the sample keeps each endpoint and planner reason visible instead of simulating support.
