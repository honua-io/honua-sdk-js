...
async function assertStaticJourney(page, sample, failures) {
  ...
  if (journey.kind === "coverage") return assertCoverageStaticJourney(page, sample, journey, failures);
...
async function assertCoverageStaticJourney(page, sample, journey, failures) {
  await page.waitForFunction(
    ({ state, readyField }) => Boolean(window[state]?.[readyField]),
    { state: journey.state, readyField: journey.readyField },
    { timeout: 10_000 },
  ...
  const proof = await page.evaluate(async ({ state, expected }) => {
    const runtime = window[state];
    const map = window.__honuaMaps?.[0];
    if (!runtime?.ready || !map) throw new Error("Coverage runtime and MapLibre map did not become ready.");
    const mounted = (protocol) => ({
      protocol: runtime.activeProtocol,
      sourceId: runtime.mapSourceId,
      sourceMounted: Boolean(map.getSource(expected[`${protocol}SourceId`])),
      layerMounted: Boolean(map.getLayer(expected[`${protocol}LayerId`])),
    });
    const initialObjectUrl = runtime.activeObjectUrl;
    const ogc = {
      mounted: mounted("ogc"),
      cancellation: await runtime.proveCancellation(),
      degradation: await runtime.proveDegradation(),
    };
    runtime.selectProtocol("wcs");
    await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    const wcsObjectUrl = runtime.activeObjectUrl;
    const wcs = {
      mounted: mounted("wcs"),
      cancellation: await runtime.proveCancellation(),
      degradation: await runtime.proveDegradation(),
    };
    const requests = runtime.requests.map((value) => new URL(value));
    const requestProof = {
      allVirtualFixture: requests.every((url) => url.origin === "https://coverages.fixture.invalid"),
      ogcSuccess: requests.some(
        (url) => url.pathname.endsWith("/coverage") && url.searchParams.get("properties") === "elevation",
      ),
      ogcCancellation: requests.some(
        (url) => url.pathname.endsWith("/coverage") && url.searchParams.get("properties") === "quality",
      ),
      ogcDegradation: requests.some(
        (url) => url.pathname.endsWith("/coverage") && url.searchParams.get("properties") === "not-a-band",
      ),
      wcsSuccess: requests.some(
        (url) => url.searchParams.get("REQUEST") === "GetCoverage" && url.searchParams.get("RANGESUBSET") === "elevation",
      ),
      wcsCancellation: requests.some(
        (url) => url.searchParams.get("REQUEST") === "GetCoverage" && url.searchParams.get("RANGESUBSET") === "quality",
      ),
      wcsDegradation: requests.some(
        (url) => url.searchParams.get("REQUEST") === "GetCoverage" && url.searchParams.get("RANGESUBSET") === "not-a-band",
      ),
    };
    const beforeDispose = {
      ready: runtime.ready,
      ...
      cancellationCount: runtime.cancellationCount,
      degradationCount: runtime.degradationCount,
      requestCount: runtime.requestCount,
      objectUrlsUnique: initialObjectUrl !== wcsObjectUrl,
      switchedObjectUrlRevoked: runtime.revokedObjectUrls.includes(initialObjectUrl),
      activeObjectUrl: wcsObjectUrl,
      ...wcs.mounted,
    };
    runtime.dispose();
    return {
      ...
      disposal: {
        ...
        ready: runtime.ready,
        sourceId: runtime.mapSourceId,
        activeObjectUrl: runtime.activeObjectUrl,
        sourceCleanupVerified: runtime.sourceCleanupVerified,
        mapRemoved: runtime.mapRemoved,
        canvasCount: document.querySelectorAll(expected.canvasSelector).length,
        revokedBothObjectUrls:
          runtime.revokedObjectUrls.includes(initialObjectUrl) && runtime.revokedObjectUrls.includes(wcsObjectUrl),
        revokedObjectUrlCount: new Set(runtime.revokedObjectUrls).size,
      },
    };
  }, { state: journey.state, expected: journey });

  const expectedOgc = {
    protocol: "ogc",
    sourceId: journey.ogcSourceId,
    sourceMounted: true,
    layerMounted: true,
  };
  const expectedWcs = {
    protocol: "wcs",
    sourceId: journey.wcsSourceId,
    sourceMounted: true,
    layerMounted: true,
  };
  if (JSON.stringify(proof.ogc.mounted) !== JSON.stringify(expectedOgc))
    failures.push(`${sample.id}: exact OGC source/layer was not mounted: ${JSON.stringify(proof.ogc.mounted)}`);
  if (JSON.stringify(proof.wcs.mounted) !== JSON.stringify(expectedWcs))
    failures.push(`${sample.id}: exact WCS source/layer was not mounted: ${JSON.stringify(proof.wcs.mounted)}`);
  for (const [protocol, result] of [["ogc", proof.ogc], ["wcs", proof.wcs]]) {
    if (result.cancellation?.status !== "cancelled" || result.cancellation?.activeProtocol !== protocol)
      failures.push(`${sample.id}: ${protocol.toUpperCase()} cancellation was not proven`);
    if (
      result.degradation?.status !== "degraded" ||
      result.degradation?.code !== "InvalidParameterValue" ||
      result.degradation?.activeProtocol !== protocol
    )
      failures.push(`${sample.id}: ${protocol.toUpperCase()} degradation was not proven`);
  }
  if (Object.values(proof.requestProof).some((value) => value !== true))
    failures.push(`${sample.id}: protocol-specific request evidence is incomplete: ${JSON.stringify(proof.requestProof)}`);
  const before = proof.beforeDispose;
  if (
    !before.ready ||
    before.phase !== "degraded" ||
    before.fixtureDigest !== journey.fixtureDigest ||
    before.ogcByteLength !== journey.fixtureByteLength ||
    before.wcsByteLength !== journey.fixtureByteLength ||
    before.imageWidth !== journey.imageWidth ||
    before.imageHeight !== journey.imageHeight ||
    before.centerPixelValue !== journey.centerPixelValue ||
    JSON.stringify(before.centerPixelColor) !== JSON.stringify(journey.centerPixelColor) ||
    before.cancellationCount !== 2 ||
    before.degradationCount !== 2 ||
    before.requestCount !== 12 ||
    !before.objectUrlsUnique ||
    !before.switchedObjectUrlRevoked
  )
    failures.push(`${sample.id}: decoded fixture or resilience evidence drifted: ${JSON.stringify(before)}`);
  if (
    visibleEvidence.canvasCount !== 1 ||
    !visibleEvidence.legend.includes(journey.legendMinText) ||
    !visibleEvidence.legend.includes(journey.legendMaxText) ||
    visibleEvidence.pixel !== journey.pixelText
  )
    failures.push(`${sample.id}: visible canvas, legend, or pixel evidence drifted: ${JSON.stringify(visibleEvidence)}`);
  if (
    !proof.disposal.disposed ||
    proof.disposal.ready ||
    proof.disposal.sourceId !== null ||
    proof.disposal.activeObjectUrl !== null ||
    !proof.disposal.sourceCleanupVerified ||
    !proof.disposal.mapRemoved ||
    proof.disposal.canvasCount !== 0 ||
    !proof.disposal.revokedBothObjectUrls ||
    proof.disposal.revokedObjectUrlCount !== 2
  )
    failures.push(`${sample.id}: source/map/object-URL disposal is incomplete: ${JSON.stringify(proof.disposal)}`);
  return { ...proof, visibleEvidence };
}
