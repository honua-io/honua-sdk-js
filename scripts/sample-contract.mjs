...
const LEGACY_VISUAL_MAX_ARTIFACT_BLOBS = 160;
const LEGACY_VISUAL_MAX_ARTIFACT_BLOB_BYTES = 12 * 1024 * 1024;
const LEGACY_VISUAL_MAX_ARTIFACT_ENCODED_BYTES = 24 * 1024 * 1024;
const LEGACY_VISUAL_MAX_ARTIFACT_DECODED_BYTES = 24 * 1024 * 1024;
const LEGACY_VISUAL_MAX_ARTIFACT_REFERENCED_BYTES = 64 * 1024 * 1024;
...
export async function migrateCatalogV1ToV2(catalog, migration) {
  ...
  const migratedSamples = await Promise.all(catalog.samples.map(async (sample) => {
    ...
    return {
      ...
      // Generated by scripts/sample-playgrounds.mjs into the overlay; carried
      // through so the catalog names the playground a reader can open (#958).
      ...(override.playground ? { playground: structuredClone(override.playground) } : {}),
