...
function migrationRuntimeFixture(manifest) {
  ...
  const runtimeMigrations = manifest.migrations.filter((entry) => entry.kind === "runtime");
  const rootMigrationCount = runtimeMigrations.filter((entry) => entry.replacement === packageJson.name).length;
  let index = 0;
  let rootIndex = runtimeMigrations.length - rootMigrationCount;
  const aliases = [];
  for (const [replacement, migrations] of groupImports(manifest.migrations, "runtime")) {
    const imports = migrations
      .sort((a, b) => (a.symbol < b.symbol ? -1 : a.symbol > b.symbol ? 1 : 0))
      .map((migration) => {
        const aliasIndex = replacement === packageJson.name ? rootIndex++ : index++;
        const alias = `movedRuntime${String(aliasIndex).padStart(4, "0")}`;
        aliases.push(alias);
        return `${migration.replacementSymbol ?? migration.symbol} as ${alias}`;
      });
...
function migrationTypeFixture(manifest) {
  const lines = [
    "/** Generated by scripts/root-surface.mjs. Every moved type-only symbol is re-exported from its replacement. */",
  ];
  const typeMigrations = manifest.migrations.filter((entry) => entry.kind === "type");
  const rootMigrationCount = typeMigrations.filter((entry) => entry.replacement === packageJson.name).length;
  let index = 0;
  let rootIndex = typeMigrations.length - rootMigrationCount;
  for (const [replacement, migrations] of groupImports(manifest.migrations, "type")) {
    const imports = migrations
      .sort((a, b) => (a.symbol < b.symbol ? -1 : a.symbol > b.symbol ? 1 : 0))
      .map((migration) => {
        const aliasIndex = replacement === packageJson.name ? rootIndex++ : index++;
        const alias = `MovedType${String(aliasIndex).padStart(4, "0")}`;
        return `${migration.replacementSymbol ?? migration.symbol} as ${alias}`;
      });
    const inline = `export type { ${imports.join(", ")} } from "${replacement}";`;
    lines.push(
      inline.length <= 120 ? inline : `export type {\n  ${imports.join(",\n  ")},\n} from "${replacement}";`,
    );
  }
  // Module resolution itself is the compatibility proof: `tsc` fails when a
  // replacement subpath stops exporting a moved symbol. The fixture re-exports
  // rather than imports because referencing the aliases as bare types would
  // require inventing arguments for generic APIs, and a plain import would be
  // an unused binding under `noUnusedImports` / `noUnusedLocals`.
  lines.push("");
  return lines.join("\n");
}
...
function migrationDoc(manifest) {
  ...
  const lines = [
    ...
    ...manifest.migrations.map((entry) => {
      const replacement = entry.replacementSymbol
        ? `\`${entry.replacementSymbol}\` from \`${entry.replacement}\``
        : `\`${entry.replacement}\``;
      return `| \`${entry.symbol}\` | ${entry.kind} | ${replacement} |`;
    }),
    "",
  ];
  return lines.join("\n");
...
async function check() {
  ...
  for (const migration of manifest.migrations) {
    const replacementSymbol = migration.replacementSymbol ?? migration.symbol;
    const available =
      migration.kind === "runtime"
        ? replacementRuntime.get(migration.replacement)?.has(replacementSymbol)
        : replacementDeclarations.get(migration.replacement)?.has(replacementSymbol);
    if (!available) {
      failures.push(`${migration.replacement} does not export replacement ${migration.kind} ${replacementSymbol}`);
    }
  }
