...
function exactKeys(value, keys, label) {
  ...
  invariant(JSON.stringify(actual) === JSON.stringify(expected), `${label} has an unexpected schema`);
}
...
export function validateSmokeReceipt(smoke, manifest, sourceCommit, sourceDateEpoch) {
  exactKeys(smoke, ['format', 'generatedAt', 'manifest', 'summary', 'results'], 'Smoke receipt');
  invariant(smoke.format === 'honua.sdk.sample-bundle-browser-smoke.v1', 'Unexpected smoke format');
  invariant(!Number.isNaN(Date.parse(smoke.generatedAt)), 'Smoke generatedAt must be an ISO timestamp');
  exactKeys(smoke.manifest, ['format', 'schemaVersion', 'commit'], 'Smoke manifest identity');
  invariant(smoke.manifest.format === manifest.format, 'Smoke manifest format mismatch');
  invariant(smoke.manifest.schemaVersion === manifest.schemaVersion, 'Smoke schema version mismatch');
  invariant(smoke.manifest.commit === sourceCommit, 'Smoke source commit mismatch');
  ...
  invariant(smoke.summary.total === smoke.results.length, 'Smoke total does not match its results');
  invariant(smoke.summary.passed === smoke.results.length, 'Every smoke result must pass');
  ...
  for (const result of smoke.results) {
    exactKeys(
      result,
      ['id', 'title', 'passed', 'requestCount', 'network', 'staticJourney', 'liveProbe', 'failures', 'screenshot'],
      `Smoke result ${result.id}`,
    );
    invariant(result.passed === true, `Smoke result did not pass: ${result.id}`);
    invariant(Number.isSafeInteger(result.requestCount) && result.requestCount > 0, `Invalid request count: ${result.id}`);
    exactKeys(result.network, ['offOriginRequestCount', 'clientErrorResponseCount'], `Smoke network ${result.id}`);
    invariant(result.network.offOriginRequestCount === 0, `Off-origin smoke request: ${result.id}`);
    invariant(result.network.clientErrorResponseCount === 0, `Client error in smoke: ${result.id}`);
    invariant(Array.isArray(result.failures) && result.failures.length === 0, `Smoke failures present: ${result.id}`);
    invariant(result.screenshot === null, `Unexpected smoke screenshot: ${result.id}`);
  }
  return {
    format: smoke.format,
...
export async function createAttestationReceipts({
  ...
  invariant(workflowSha === sourceCommit, 'Workflow and source commits must be identical');
  invariant(/^\d+$/.test(workflowRunId), 'Workflow run id must be numeric');
  invariant(/^\d+$/.test(workflowRunAttempt), 'Workflow run attempt must be numeric');
  invariant(Number.isSafeInteger(sourceDateEpoch) && sourceDateEpoch > 0, 'Invalid source date epoch');
  for (const [label, value] of Object.entries({ runnerImage, runnerImageVersion, runnerEnvironment, runnerOs, runnerArch })) {
  ...
  const [firstManifest, secondManifest, firstArchive, secondArchive, lockfile, rawSmoke] =
    await Promise.all([
  ...
  const receipt = {
    ...
    workflow: { path: WORKFLOW_PATH, commit: workflowSha, actions: actionIdentity },
    build: {
      cleanBuildCount: 2,
    ...
    publication: {
      ...
      githubCli: {
        version: '2.93.0',
        linuxAmd64ArchiveSha256: '02d1290eba130e0b896f3709ffff22e1c75a51475ddb70476a85abc6b5807af0',
      },
    },
    attestation: {
  ...
  const immutableReceiptBytes = Buffer.from(`${JSON.stringify(receipt, null, 2)}\n`);
  const runReceipt = {
...
export function classifyPublicationState(state, expected) {
  invariant(expected?.sourceCommit && expected?.assets, 'Expected publication state is required');
  ...
  invariant(JSON.stringify(actualNames) === JSON.stringify(expectedNames), 'Release asset set differs');
  for (const asset of state.release.assets) {
    const expectedAsset = expected.assets[asset.name];
    invariant(asset.bytes === expectedAsset.bytes, `Release asset size differs: ${asset.name}`);
    invariant(asset.sha256 === expectedAsset.sha256, `Release asset digest differs: ${asset.name}`);
  }
  return 'idempotent';
}
...
export function validateContentAddressedWorkflowPolicy(workflow) {
  ...
  invariant(/^permissions: \{\}$/m.test(workflow), 'Global permissions must be empty');
  invariant(workflow.includes('group: publish-content-addressed-sample-bundles'), 'Concurrency must be global');
  invariant(workflow.includes('SOURCE_COMMIT: ${{ github.sha }}'), 'Source commit must be github.sha');
  invariant(workflow.includes('RELEASE_ID: sample-bundles-${{ github.sha }}'), 'Release id must derive from github.sha');
  invariant(workflow.includes('runs-on: ubuntu-24.04'), 'Runner must be pinned');
  invariant(workflow.includes(`node-version: '${REQUIRED_NODE_VERSION}'`), 'Node must be pinned');
  invariant(!workflow.includes('sample-bundles-latest'), 'Rolling tags are forbidden');
  invariant(!workflow.includes('--clobber') && !/overwrite:\s*true/.test(workflow), 'Overwrite is forbidden');
  invariant(!workflow.includes('GITHUB_ENV') && !workflow.includes('GITHUB_PATH'), 'Environment/path transfer is forbidden');
  invariant(!/tar\s+-[^\s\n]*c[^\s\n]*f/.test(workflow), 'Ad hoc bundle packing is forbidden');
  const uses = [...workflow.matchAll(/^\s*-?\s*uses:\s*([^\s#]+).*$/gm)].map((match) => match[1]);
  invariant(uses.length >= 7, 'Expected all governed actions');
  for (const action of uses) invariant(/^[^@]+@[a-f0-9]{40}$/.test(action), `Unpinned action: ${action}`);
  for (const [name, pin] of Object.entries(ACTION_PINS)) {
    ...
    invariant(uses.includes(`${actionName}@${pin}`), `Missing verified ${actionName} pin`);
  }
  invariant((workflow.match(/persist-credentials:\s*false/g) ?? []).length === 3, 'Three credentialless checkouts required');
  invariant((workflow.match(/ref: \$\{\{ github\.sha \}\}/g) ?? []).length === 3, 'All checkouts must use github.sha');
  invariant((workflow.match(/refs\/heads\/trunk/g) ?? []).length >= 3, 'Trunk gates and attestation ref required');
  invariant((workflow.match(/commits\/trunk/g) ?? []).length >= 2, 'Current trunk must be checked before build and publish');
  invariant((workflow.match(/samples:bundles:build/g) ?? []).length >= 2, 'Two builds required');
  invariant((workflow.match(/pack-sample-bundles\.mjs/g) ?? []).length === 2, 'Packer must run twice');
  invariant(workflow.includes('cmp --silent "$FIRST_MANIFEST" "$SECOND_MANIFEST"'), 'Manifest cmp required');
  invariant(workflow.includes('cmp --silent "$FIRST_ARCHIVE" "$SECOND_ARCHIVE"'), 'Archive cmp required');
  invariant((workflow.match(/\/immutable-releases/g) ?? []).length >= 2, 'Immutable releases API must be checked before and after publish');
  invariant(
    (workflow.match(/\.enabled == true and \.enforced_by_owner == true/g) ?? []).length >= 2,
    'Owner enforcement must be checked before and after publish',
  );
  invariant(workflow.includes('--target "$SOURCE_COMMIT"'), 'Release target must be the source commit');
  invariant(workflow.includes('--latest=false'), 'Content-addressed release must not be latest');
  invariant((workflow.match(/attestation verify/g) ?? []).length >= 2, 'Release and run attestation verification required');
  for (const flag of ['--signer-workflow', '--signer-digest', '--source-ref', '--source-digest', '--deny-self-hosted-runners', '--predicate-type']) {
    invariant(workflow.includes(flag), `Attestation verification must enforce ${flag}`);
  }
  const buildStart = workflow.indexOf('  build-and-smoke:');
  const publishStart = workflow.indexOf('  attest-and-publish:');
  invariant(buildStart > 0 && publishStart > buildStart, 'Separate build and publish jobs required');
  const buildJob = workflow.slice(buildStart, publishStart);
  const publishJob = workflow.slice(publishStart);
  invariant(/permissions:\n      contents: read/.test(buildJob), 'Build job must be read-only');
  invariant(buildJob.includes('commits/trunk'), 'Build job must confirm current trunk');
  invariant(buildJob.includes('test "$GITHUB_REF" = "refs/heads/trunk"'), 'Build job must require the trunk ref');
  invariant(!/contents: write|id-token: write|attestations: write/.test(buildJob), 'Build job is overprivileged');
  invariant(/contents: write/.test(publishJob) && /id-token: write/.test(publishJob) && /attestations: write/.test(publishJob), 'Publish permissions missing');
  invariant(!/actions\/checkout@|\bnpm\b|\bnode\b|governance\/scripts|source-a|source-b/.test(publishJob), 'Privileged job executes repository/build code');
  invariant(publishJob.includes('test "$GITHUB_REF" = "refs/heads/trunk"'), 'Publish job must require the trunk ref');
  invariant(publishJob.indexOf('commits/trunk') < publishJob.indexOf('Attest exact immutable publication'), 'Publish job must recheck current trunk before attesting');
  invariant(publishJob.includes('actions/download-artifact@'), 'Privileged job must redownload staged bytes');
  invariant((publishJob.match(/actions\/attest-build-provenance@/g) ?? []).length === 2, 'Release and run receipts require separate attestations');
  invariant(publishJob.indexOf('Validate staged bytes without credentials') < publishJob.indexOf('Expose write token'), 'Validation must precede token exposure');
  return true;
}
