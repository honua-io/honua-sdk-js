...
async function measureBundle(entryAbs, extraExternal = []) {
  ...
  return { min: buffer.byteLength, gzip: gzipBytes(buffer), inputs: retainedMetafileInputs(result.metafile) };
}

function measurePrebuilt(entryAbs) {
  const buffer = fs.readFileSync(entryAbs);
  return { min: buffer.byteLength, gzip: gzipBytes(buffer), inputs: [] };
}
...
async function measureAll() {
  ...
  for (const target of TARGETS) {
    ...
    const requiredModules = fixture?.requiredModules ?? (ERROR_LEAF_TARGET_KEYS.has(target.key) ? ["dist/src/core/error-base.js"] : []);
    const forbiddenModules = fixture?.forbiddenModules ?? (ERROR_LEAF_TARGET_KEYS.has(target.key) ? ERROR_LEAF_FORBIDDEN_MODULES : []);
...
/** Pure #583 contractual comparison: every named target must be at least 25% below its admitted baseline. */
...
function renderErrorEvidence(measurements, nfrRows) {
  ...
  for (const row of nfrRows) {
    ...
    lines.push(
      `| \`${row.key}\` | ${row.baseline} B | ${row.measured} B | ${(row.reduction * 100).toFixed(2)}% | ${row.ceiling} B | ${modules} |`,
    );
  }
