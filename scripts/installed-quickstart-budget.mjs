...
try {
  ...
  async function measure(label, removePeerAlias = false) {
    ...
    return { status: failure ? "failed" : "passed", ...(failure ? { diagnostic: failure } : {}),
  ...
  assert.equal(corrected.status, "passed", corrected.diagnostic);
  ...
  const receipt = { schema: "honua.installed-first-map-budget/v1", generatedAt: new Date().toISOString(),
    ...
    node: process.version, ...(baseline ? { baseline } : {}), corrected };
