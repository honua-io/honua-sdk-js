...
describe("GeoParquet lossless DuckDB value decoder", () => {
  ...
  it("rejects hostile schema metadata without invoking accessors or exposing thrown values", () => {
    let reads = 0;
    const accessorField = { type: "BIGINT" } as { name?: string; type: string };
    Object.defineProperty(accessorField, "name", {
      enumerable: true,
      get() {
        reads++;
        throw new Error("schema-accessor-secret");
      },
    });
    const accessorError = expectFailure(
      () => compileDuckDbLosslessRowDecoder([accessorField as never]),
      "GEOPARQUET_LOSSLESS_ACCESSOR",
    );
    expect(accessorError.path).toBe("$.fields[0].name");
    expect(accessorError.message).not.toContain("schema-accessor-secret");
    expect(reads).toBe(0);
    ...
    const proxyError = expectFailure(
      () => compileDuckDbLosslessRowDecoder([proxyField as never]),
      "GEOPARQUET_LOSSLESS_AMBIGUOUS_WRAPPER",
    );
    expect(proxyError.message).not.toContain("schema-proxy-secret");
  });
  ...
  const invalidCases: ReadonlyArray<{
    ...
    {
      label: "floating-point bytes presented as an exact integer wrapper",
      type: "BIGINT",
      value: new Float64Array([1]),
      code: "GEOPARQUET_LOSSLESS_AMBIGUOUS_WRAPPER",
    },
    {
      label: "multi-byte typed array presented as binary",
      type: "BLOB",
