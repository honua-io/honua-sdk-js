...
// Derived-artifact decoupling (honua-io/honua-sdk-js#677): the migration
// workbench artifacts under examples/migration-workbench/public/artifacts are
// regenerated on trunk. At PR time (relax signal set) we do not require the
// committed artifacts to already match the freshly generated bytes; the
// generator's determinism, no-path-leak, internal manifest integrity, and
// fresh provenance are all still asserted below. The committed-vs-fresh byte
// equality is re-enforced strictly on trunk.
const DERIVED_ARTIFACTS_RELAXED = /^(1|true|yes|on)$/i.test(process.env.HONUA_DERIVED_ARTIFACTS_RELAX ?? "");
...
describe("migration workbench artifact supply chain", () => {
  it("is byte-deterministic across different temporary roots and matches committed output", async () => {
    ...
    for (const [repositoryPath, firstBytes] of first.artifacts) {
      ...
      if (!DERIVED_ARTIFACTS_RELAXED) {
        expect(fs.readFileSync(path.join(repositoryRoot, repositoryPath)).equals(firstBytes), repositoryPath).toBe(
          true,
        );
      }
