...
describe("capability-to-sample matrix contract", () => {
  ...
  it("deterministically projects all four authorities without inventing qualification", async () => {
    ...
    expect(matrix.supportClaims).toHaveLength(inputs.supportTruth.supportClaims.length);
    expect(matrix.packageEntrypoints).toHaveLength(Object.keys(inputs.packageJson.exports).length);
