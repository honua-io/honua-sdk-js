//#region .tmp/sample-runner/2b6110a5-4259-47a4-adbc-506c04d46ed1/packed-sdk/extract/package/dist/src/core/error-classifications.js
/** @internal */
...
//#endregion
//#region .tmp/sample-runner/2b6110a5-4259-47a4-adbc-506c04d46ed1/packed-sdk/extract/package/dist/src/core/error-envelope.js
/**
...
function isTerminalFailureReceipt(value) {
	return isRecord(value) && typeof ownDataProperty(value, "kind") === "string" && typeof ownDataProperty(value, "retryable") === "boolean" && Array.isArray(ownDataProperty(value, "fieldErrors")) && isRecord(ownDataProperty(value, "protocolMetadata"));
}
...
//#endregion
//#region .tmp/sample-runner/2b6110a5-4259-47a4-adbc-506c04d46ed1/packed-sdk/extract/package/dist/src/core/errors.js
/**
...
*/
var HonuaGeometryError = class extends HonuaSdkError {
	code;
	detail;
...
var HTTP_RETRYABLE_STATUSES = /* @__PURE__ */ new Set([
	408,
	429,
	500,
	502,
...
var GRPC_RETRYABLE_CODES = /* @__PURE__ */ new Set([
	4,
	8,
	10,
	14
]);
function httpFailureReceipt(statusCode, body, options) {
	const root = isObject(body) ? body : void 0;
	const protocolError = root && isObject(root.error) ? root.error : void 0;
	...
	const code = stringValue(source?.machineCode) ?? stringValue(source?.code) ?? stringValue(root?.code) ?? defaultCode(kind);
	...
	const correlationId = stringValue(source?.correlationId) ?? stringValue(root?.correlationId) ?? firstMetadata(metadata, "x-correlation-id", "honua-request-id", "x-request-id", "honua-correlation-id");
	const fieldErrors = parseFieldFailures(source?.errors ?? root?.errors);
	return freezeReceipt({
		transportStatus: options.transportStatus ?? statusCode,
		...protocolCode !== void 0 ? { protocolCode } : {},
		kind,
		...code ? { code } : {},
		retryable: explicitRetryable ?? (HTTP_RETRYABLE_STATUSES.has(classificationCode) || HTTP_RETRYABLE_STATUSES.has(options.transportStatus ?? statusCode)),
		...retryAfterSeconds !== void 0 ? { retryAfterMs: Math.max(0, retryAfterSeconds * 1e3) } : headerDelay !== void 0 ? { retryAfterMs: headerDelay } : {},
		...correlationId ? { correlationId } : {},
		fieldErrors,
		protocolMetadata: {
			initial: metadata,
			trailing: emptyMetadata()
		}
	});
}
function grpcFailureReceipt(code, details, options) {
	...
	const declaredRetryable = firstMetadata(trailing, "honua-retryable", "honua-error-retryable") ?? firstMetadata(initial, "honua-retryable", "honua-error-retryable");
	const retryAfterMs = parseRetryAfter(metadataGetterFromRecords(trailing, initial));
	const correlationId = firstMetadata(trailing, "x-correlation-id", "honua-request-id", "x-request-id", "honua-correlation-id") ?? firstMetadata(initial, "x-correlation-id", "honua-request-id", "x-request-id", "honua-correlation-id");
...
function failureKindForHttp(code, transportStatus) {
...
function failureKindForGrpc(code) {
...
function metadataRecord(value) {
	...
	const add = (key, item) => {
		...
		const values = Array.isArray(item) ? item.filter((entry) => typeof entry === "string") : [String(item)];
		if (values.length > 0) result[normalized] = Object.freeze(values.slice(0, 20));
	};
	if (value && typeof value.entries === "function") for (const [key, item] of value.entries()) add(key, item);
	else if (isObject(value)) for (const [key, item] of Object.entries(value)) add(key, item);
	return Object.freeze(result);
}
function metadataGetter(value) {
	if (value && typeof value.get === "function") return value;
	return metadataGetterFromRecords(metadataRecord(value));
}
function metadataGetterFromRecords(...records) {
	return { get(name) {
...
function firstMetadata(record, ...keys) {
...
function parseRetryAfter(headers) {
	...
	return Number.isFinite(target) ? Math.max(0, target - Date.now()) : void 0;
}
function parseEncodedFieldFailures(encoded, details) {
	if (encoded) try {
		return parseFieldFailures(JSON.parse(encoded));
	} catch {
		return [];
	}
	return parseFieldFailures(details);
}
function parseFieldFailures(value) {
	if (!Array.isArray(value)) return Object.freeze([]);
	return Object.freeze(value.filter(isObject).map((item) => {
...
function isFailureKind(value) {
	return value !== void 0 && FAILURE_KINDS.has(value);
}
...
//#endregion
//#region .tmp/sample-runner/2b6110a5-4259-47a4-adbc-506c04d46ed1/packed-sdk/extract/package/dist/src/core/geometry-input-sr.js
/** GeoServices query geometry defaults to WGS84, never the target layer CRS. */
function geometryInputSpatialReference(geometry) {
	if (typeof geometry === "string") try {
	...
	if (typeof sr === "object" && sr !== null) {
		...
		if ("latestWkid" in sr && typeof sr.latestWkid === "number" && Number.isInteger(sr.latestWkid) && sr.latestWkid > 0) return sr.latestWkid;
...
//#endregion
export { HonuaHttpError as a, HonuaWfsExceptionError as c, HonuaGrpcError as i, HonuaSdkError as l, HonuaAbortError as n, HonuaNetworkError as o, HonuaCapabilityNotSupportedError as r, HonuaTimeoutError as s, geometryInputSpatialReference as t };
