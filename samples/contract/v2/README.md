...
refreshes the canonical live observation and catalog expiry, commits that
source snapshot, then regenerates, verifies, and commits its complete receipt
set twice a week. It pushes neither commit unless the entire transaction
passes; a failed refresh remains visible while the prior set is still current.
