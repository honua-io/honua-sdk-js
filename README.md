...
by an API-surface gate; 10 experimental subpaths may change before 1.0, and
...
- Symbols marked `@experimental` in JSDoc may change in any minor release. The full table of
  ...
  - **Experimental subpath-only APIs** (not re-exported from the root barrels):
    `/nl-map-control`, `/geoparquet`, `/source-schema`, `/source-capabilities`, `/plugin`, `/deckgl`,
    `/offline`, `/diagnostics`, `/routing` — with `/query-planner` below, 10 experimental subpaths in total.
