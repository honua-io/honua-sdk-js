...
- Symbols marked `@experimental` in JSDoc may change in any minor release. The full table of
  ...
  - **Experimental subpath-only APIs** (not re-exported from the root barrels):
    `/nl-map-control`, `/geoparquet`, `/source-schema`, `/source-capabilities`, `/source-capability-discovery`, `/stac-discovery`, `/plugin`, `/deckgl`,
    `/offline`, `/diagnostics`, `/routing` — with `/query-planner` below, 12 experimental subpaths in total.
