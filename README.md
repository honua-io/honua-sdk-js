...
| `@honua/sdk-js` (root) | 202.3 KiB |
| `{ HonuaClient }` from the root (tree-shake guard) | 62.0 KiB |
| `{ connect }` from the root (tree-shake guard) | 163.8 KiB |
| `{ createHonua }` from the root (tree-shake guard) | 192.7 KiB |

The root is the whole reviewed kernel and the guards price its verbs honestly: importing `{ connect }`
alone costs 163.8 KiB gzip and `{ createHonua }` 192.7 KiB against the 202.3 KiB root, so size-sensitive
