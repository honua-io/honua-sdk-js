...
cleanup.listen(cancelButton, "click", () => truth.cancel());
...
cleanup.listen(sourceList, "click", (event) => {
  ...
  if (!(target instanceof HTMLButtonElement) || !target.dataset.sourceId) return;
  sourceInput.value = target.dataset.sourceId;
...
function renderTruth(state: ServiceExplorerTruthState): void {
  ...
  runtime.ready = state.kind !== "idle" && state.kind !== "loading";
...
function renderQueryPlan(projection: ServiceExplorerOperationProjection): void {
  ...
  if (!plan) steps.append(message("No accepted query plan.", "empty-state"));
}
...
async function runAcceptedQuery(): Promise<void> {
  ...
  try {
    ...
    await applyQueryResult(projection, execution.result);
    ...
    if (!controller.signal.aborted) showOperationFailure(error, "query execution failed");
  } finally {
...
async function renderAcceptedSource(): Promise<void> {
  ...
  try {
    if (plan.selected.strategy === "geojson-query") {
      ...
      await applyNativePlan(plan);
    }
    runtime.interactionCount += 1;
    setText("#map-status", `${plan.selected.strategy} mounted`);
    showRenderPlan(plan);
  } catch (error) {
    if (!controller.signal.aborted) showOperationFailure(error, "render preparation failed");
  } finally {
    if (operationController === controller) operationController = undefined;
    refreshOperationProjection();
  }
...
async function applyNativePlan(plan: AutomaticMapLibrePlan): Promise<void> {
...
function waitForMapStyle(): Promise<void> {
