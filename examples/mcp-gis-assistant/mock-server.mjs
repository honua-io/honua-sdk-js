...
function buildDemoIfNeeded() {
  ...
  const result = spawnSync(npmCommand, ["run", "demo:mcp-gis-assistant:build", "--silent"], {
    cwd: projectRoot,
    stdio: "inherit",
    env: process.env,
  });
  if (result.status !== 0) throw new Error("Failed to build the MCP GIS Assistant sample.");
}
...
export async function startMcpGisAssistantFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") throw new Error("Failed to bind MCP GIS Assistant fixture server.");
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`mcpGisAssistantUrl=${server.url}\n`);
}
