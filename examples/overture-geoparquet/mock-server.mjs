...
function buildDemoIfNeeded() {
  const result = runNpmScriptSync("demo:overture:build:offline", {
    cwd: projectRoot,
    stdio: "inherit",
    env: createFixtureBuildEnvironment(),
  });
  if (result.status !== 0) throw new Error("Failed to build the Overture GeoParquet Explorer sample.");
}
...
export async function startOvertureFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") throw new Error("Failed to bind Overture fixture server.");
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`overtureUrl=${server.url}\n`);
}
