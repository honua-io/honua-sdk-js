...
export function createGeocodingAuditRows(config: GeocodingQuickstartConfig): GeocodingAuditRow[] {
  const basePath = `${config.honuaBaseUrl}/rest/services/${encodeURIComponent(config.locatorName)}/GeocodeServer`;
  return [
    {
