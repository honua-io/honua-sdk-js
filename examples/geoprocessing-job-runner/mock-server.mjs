...
function buildDemoIfNeeded() {
  const result = runNpmScriptSync("demo:gp-runner:build", {
    cwd: projectRoot,
    stdio: "inherit",
...
function statusDocument(status, progress, message) {
  return {
    ...
    message,
    links: [
      { rel: "status", href: jobPath, type: "application/json" },
      { rel: "results", href: resultsPath, type: "application/json" },
    ],
  };
}
function resultArtifact() {
  return {
    ...
    href: `data:application/geo+json,${encodeURIComponent(JSON.stringify(fixture.resultFeature))}`,
...
export async function startGeoprocessingJobRunnerFixtureServer({ build = true } = {}) {
      ...
      if (method === "POST" && requestUrl.pathname === executionPath) {
        ...
        if (!isDeepStrictEqual(body, expectedBody)) {
          sendJson(res, 422, {
            ...
            status: 422,
            detail: "Invalid buffer inputs: pinned fixture inputs did not match geometry.buffer's admitted schema.",
          });
          return;
        }
        statusPolls = 0;
        dismissed = false;
        sendJson(res, 201, statusDocument("accepted", 5, "Buffer job accepted"), { location: jobPath });
        return;
      }
      ...
      if (method === "GET" && requestUrl.pathname === resultsPath) {
        ...
        if (dismissed)
          sendJson(res, 410, {
            ...
            title: "Job dismissed",
            status: 410,
            detail: "The fixture job was dismissed.",
          });
        else if (statusPolls < 2)
          sendJson(res, 404, {
            type: "result-not-ready",
            title: "Result not ready",
            status: 404,
            detail: "The fixture job is not terminal.",
          });
        else sendJson(res, 200, { output1: resultArtifact() });
        return;
      }
      if (method === "DELETE" && requestUrl.pathname === jobPath) {
        requests.push({ method, path: requestUrl.pathname });
        dismissed = true;
        sendJson(res, 200, statusDocument("dismissed", 100, "Dismissed via OGC API"));
        return;
      }
  ...
  if (!address || typeof address === "string")
    throw new Error("Failed to bind Geoprocessing Job Runner fixture server.");
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`geoprocessingJobRunnerUrl=${server.url}\n`);
}
