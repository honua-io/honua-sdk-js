...
function buildDemoIfNeeded() {
  ...
  const result = spawnSync(npmCommand, ["run", "demo:edit-workflow:build", "--silent"], {
    cwd: projectRoot,
    stdio: "inherit",
    env: process.env,
...
export async function startEditWorkflowFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Failed to bind Edit Workflow fixture server.");
  }
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`editWorkflowUrl=${server.url}\n`);
}
