...
export interface PlanningSearchReceipt {
...
export interface PlanningAnalysisReceipt {
...
export interface PlanningSubmissionReceipt {
...
export interface PlanningReviewArtifact {
  ...
  readonly semantic: {
    ...
    readonly publicSurfaces: readonly ["kernel-query", "geocoding", "geometry", "edit-session", "attachments"];
    readonly failureScenarios: readonly ["invalid-domain", "conflict", "attachment-failure", "unsupported"];
  };
  readonly search: PlanningSearchReceipt;
  readonly analysis: PlanningAnalysisReceipt;
  readonly submissions: readonly PlanningSubmissionReceipt[];
}
...
export const PLANNING_EDIT_FIELDS: readonly EditWorkflowField[] = [
  { name: "OBJECTID", type: "esriFieldTypeOID", alias: "Object ID", nullable: false, editable: false },
  { name: "permit_no", type: "esriFieldTypeString", alias: "Permit number", nullable: false, length: 32 },
  { name: "parcel_tmk", type: "esriFieldTypeString", alias: "Parcel TMK", nullable: false, length: 32 },
  { name: "address", type: "esriFieldTypeString", alias: "Address", nullable: false, length: 120 },
  { name: "zoning", type: "esriFieldTypeString", alias: "Zoning", nullable: false, length: 16 },
  { name: "flood_zone", type: "esriFieldTypeString", alias: "Flood zone", nullable: false, length: 16 },
  {
    name: "permit_type",
    type: "esriFieldTypeString",
    alias: "Permit type",
    nullable: false,
    domain: PERMIT_TYPE_DOMAIN,
  },
  { name: "status", type: "esriFieldTypeString", alias: "Status", nullable: false, domain: STATUS_DOMAIN },
  { name: "description", type: "esriFieldTypeString", alias: "Description", nullable: false, length: 240 },
  {
  ...
  { name: "version", type: "esriFieldTypeInteger", alias: "Version", nullable: true },
  { name: "last_edited_date", type: "esriFieldTypeDate", alias: "Last edited", nullable: true, editable: false },
];
...
/**
 * Headless, deterministic golden journey. The browser sample is intentionally
 * a thin adapter over this class; no application-shell state is required to
 * exercise the public Dataset -> Source contract.
 */
export class PlanningPermittingJourney {
  ...
  #search: PlanningSearchReceipt | undefined;
  #analysis: PlanningAnalysisReceipt | undefined;
  readonly #submissions: PlanningSubmissionReceipt[] = [];
  ...
  public static async connect(options: CreatePlanningJourneyOptions): Promise<PlanningPermittingJourney> {
    ...
    try {
      ...
      const connection = await kernel.connect<PlanningRecordAttributes>(
        ...
        {
          protocol: "auto",
          authorizationScopeFingerprint: `${PLANNING_FIXTURE_VERSION}:writable`,
      ...
      const readonlyConnection = await kernel.connect<PlanningRecordAttributes>(
        ...
        {
          protocol: "auto",
          authorizationScopeFingerprint: `${PLANNING_FIXTURE_VERSION}:readonly`,
          ...(clientOptions ? { clientOptions } : {}),
        },
      );
  ...
  public async search(address: string): Promise<PlanningSearchReceipt> {
    ...
    if (!parcelTmk) throw new Error("The selected geocode candidate did not include a parcel TMK.");
    ...
    const receipt: PlanningSearchReceipt = {
      address,
      geocode,
      featureId,
  ...
  public async analyze(proposal: GeoJsonPolygon = DEFAULT_PROPOSAL): Promise<PlanningAnalysisReceipt> {
    ...
    const provenanceSources = this.#inspection.sources
      .flatMap((source) => source.provenance)
      .map((entry) => entry.source);
    const receipt: PlanningAnalysisReceipt = {
      ...
      provenance: {
        ...
        metadataSources: [...new Set(provenanceSources)],
        sourceId: this.#source.descriptor.id,
        protocol: this.#source.descriptor.protocol,
      },
      fidelity: {
        ...
        crs: "EPSG:4326",
        caveat:
          "The checked-in flood polygon is deterministic fixture evidence, not a current regulatory determination.",
      },
    };
    this.#analysis = receipt;
    return receipt;
  }

  public createDraft(proposal: GeoJsonPolygon = DEFAULT_PROPOSAL): PlanningDraft {
    const selected = this.requireSearch();
    const center = centroid(proposal) as GeoJsonPoint;
    return {
      values: {
  ...
  public async submit(draft: PlanningDraft, scenario: PlanningScenario): Promise<PlanningSubmissionReceipt> {
    const selected = this.requireSearch();
    ...
    const session = createEditSession<PlanningRecordAttributes>({
      ...
      metadata: {
        ...
        primaryKey: "OBJECTID",
        conflict: {
          state: source.capabilities.has("applyEdits") ? "supported" : "unsupported",
          versionField: "version",
          value: selected.attributes.version,
        },
      },
    ...
    const receipt: PlanningSubmissionReceipt = {
      scenario,
    ...
    this.#submissions.push(receipt);
    return receipt;
  }

  public reviewArtifact(): PlanningReviewArtifact {
  ...
  public exportReview(): string {
    return JSON.stringify(this.reviewArtifact(), null, 2);
  }

  public submissions(): readonly PlanningSubmissionReceipt[] {
  ...
  private requireSearch(): PlanningSearchReceipt {
...
function editFieldsFromMetadata(fields: readonly HonuaFieldInfo[] | undefined): readonly EditWorkflowField[] {
  if (!fields || fields.length === 0) return PLANNING_EDIT_FIELDS;
...
function editDomainFromMetadata(field: HonuaFieldInfo): EditWorkflowField["domain"] {
  ...
  if (domain.type === "codedValue") {
    ...
    return {
      ...
      codedValues: codedValues.flatMap((value) => {
...
function recoveryFor(scenario: PlanningScenario, status: EditWorkflowStatus): string {
  ...
  return "Inspect the normalized failure receipt and retry only the failed operation.";
}
