...
function buildDemoIfNeeded() {
  ...
  if (result.status !== 0) {
    throw new Error("Failed to build the planning & permitting workbench before starting the mock server.");
  }
}
...
export async function startPlanningWorkbenchFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Failed to bind the planning & permitting workbench fixture server.");
  }
