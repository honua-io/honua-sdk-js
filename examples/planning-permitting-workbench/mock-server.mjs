...
const FIELDS = [
  ...
  { name: "last_edited_date", alias: "Last edited", type: "esriFieldTypeDate", nullable: true, editable: false },
];
...
function buildDemoIfNeeded() {
  ...
  if (result.status !== 0) throw new Error("Failed to build the planning & permitting journey fixture.");
}

function layerMetadata(readonly) {
  return {
    currentVersion: 11.3,
    id: readonly ? 1 : 0,
...
function queryFeatures(state, requestUrl) {
  ...
  if (geometryText) {
    try {
      ...
      // The fixture fails closed on malformed filters instead of broadening the query.
      features = [];
    }
  }
...
async function serveApi(state, req, res, requestUrl) {
  ...
  if (pathname === `${servicePath}/0` || pathname === `${servicePath}/1`) {
    json(res, layerMetadata(pathname.endsWith("/1")), 200, { etag: '"planning-permitting-v1"' });
    return true;
  }
  ...
  if (pathname === `${geocodePath}/findAddressCandidates`) {
    ...
    const limit = Number(requestUrl.searchParams.get("maxLocations") ?? "5");
...
export async function startPlanningWorkbenchFixtureServer({ build = true } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (await serveApi(state, req, res, requestUrl)) return;
  ...
  if (!address || typeof address === "string") throw new Error("Failed to bind the planning journey fixture server.");
