...
function buildDemoIfNeeded() {
  ...
  const result = spawnSync(npmCommand, ["run", "demo:unified-ops:build", "--silent"], {
    cwd: projectRoot,
    stdio: "inherit",
  ...
  if (result.status !== 0) {
    throw new Error("Failed to build the unified ops workspace before starting the mock server.");
  }
}
...
export async function startUnifiedOpsWorkspaceFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Failed to bind the unified ops workspace fixture server.");
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const { url, close } = await startUnifiedOpsWorkspaceFixtureServer();
  process.stdout.write(`unifiedOpsWorkspaceMockUrl=${url}\n`);
