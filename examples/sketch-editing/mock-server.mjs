...
function buildDemoIfNeeded() {
  ...
  const result = spawnSync(npmCommand, ["run", "demo:sketch-editing:build", "--silent"], {
    cwd: projectRoot,
    stdio: "inherit",
    env: process.env,
  ...
  if (result.status !== 0) throw new Error("Failed to build the sketch-editing sample.");
}
...
export async function startSketchEditingFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Failed to bind sketch-editing fixture server.");
  }
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`sketchEditingUrl=${server.url}\n`);
}
