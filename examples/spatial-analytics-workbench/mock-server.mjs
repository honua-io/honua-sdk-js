...
function buildDemoIfNeeded() {
  ...
  const result = spawnSync(npmCommand, ["run", "demo:spatial-analytics:build", "--silent"], {
    cwd: projectRoot,
    stdio: "inherit",
    env: createFixtureBuildEnvironment(),
  });
  if (result.status !== 0) throw new Error("Failed to build the Spatial Analytics Workbench sample.");
}
...
export async function startSpatialAnalyticsWorkbenchFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Failed to bind Spatial Analytics Workbench fixture server.");
  }
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`spatialAnalyticsWorkbenchUrl=${server.url}\n`);
}
