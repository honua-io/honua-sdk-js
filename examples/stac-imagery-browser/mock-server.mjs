...
function buildDemoIfNeeded() {
  const result = runNpmScriptSync("demo:stac-browser:build", {
    cwd: projectRoot,
    stdio: "inherit",
...
export async function startStacBrowserFixtureServer({ build = true } = {}) {
  ...
  if (!address || typeof address === "string") throw new Error("Failed to bind STAC Browser fixture server.");
  return {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  process.stdout.write(`stacBrowserUrl=${server.url}\n`);
}
