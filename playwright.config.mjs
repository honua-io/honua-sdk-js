...
// config/browser-shards.v1.json; unset -- which is every local run, and the
// `test:maplibre-compat:prepared` lane that names its spec explicitly -- runs
// the whole directory exactly as before. Everything else about the browser
// contract (single chromium project, one worker, CI retries) is deliberately
// unchanged: sharding redistributes the work, it does not weaken it.
